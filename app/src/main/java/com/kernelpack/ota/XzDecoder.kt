 ...
 *   app/src/main/kotlin/com/ghostlock/app/data/ota/XzDecoder.kt
 ...
 *
 * [已知缺陷 —— 实测，见 XzDecoderTest]
 * 本解码器在**多块** XZ 流上会失败：实测 `xz --block-size=16384 -9` 产出的
 * 5 块流，第 1 块 16384 字节完全正确，第 2 块从**第 2 个字节**起就发散
 * （期望 `0356: KS`，实得 `00000000`），随后抛
 * `XzException: match distance beyond decoded data`。
 *
 * **没有**去修它：这是一段算术编码器，"差一位"和"完全正确"之间没有中间状态，
 * 在没有吃透它之前动手改，风险比不修大得多。目前按**限制**处理：
 *   - 单块流已用 3 条真实 `xz` 向量逐字节验证（含 crc32 / crc64 两种 check）；
 *   - 多块流由 `XzDecoderTest` 钉住"会抛异常"这一现状；
 *   - `OtaPayloadExtractor` 把它翻译成一句人能看懂的话。
 */
